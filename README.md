...
`targets`에 `food-story-expansion-v1`을 지정하면 저장된 6,768개 대본 대기열을 20개 균형 shard로 나눠 수집합니다. 1차 병렬 실행 뒤에는 `food-story-expansion-retry-v1`을 사용할 수 있습니다. 이 큐는 현재 미확보 대상을 20편 이하 shard로 동적 분할하고 최대 20개를 동시에 실행합니다. Google Sorry URL이나 HTTP 403·429는 즉시 `blocked_or_captcha`로 판정해 해당 shard를 끝내며, 명시 목록 결과는 영상마다 원자적으로 체크포인트됩니다. 한 라운드가 끝나면 병합 artifact의 `retry_targets.txt`에서 성공분과 `segments_empty`를 제외해 다음 라운드를 만든다는 전제입니다.
